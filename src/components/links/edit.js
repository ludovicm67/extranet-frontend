...
class LinksEdit extends Component {
  ...
  componentDidMount() {
    ...
    getApi(`links/${this.state.id}`).then(res => {
      ...
      state.linkData.url.final = res.url || '';
      state.linkData.title = res.title || '';
      state.linkData.description = res.description || '';
      if (res.image_url) state.linkData.img = {
        url: res.image_url,
      };
      state.categories = (res.categories)
        ? res.categories.map(e => e.id).join(',')
        : '';
