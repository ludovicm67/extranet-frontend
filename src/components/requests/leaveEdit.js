...
class Leave extends Component {
  state = {
    id: this.props.match.params.leaveId,
  ...
  componentDidMount() {
    getApi(`leave/${this.state.id}`).then(res => {
      if (this.isUnmounted) {
        return;
      }

      this.setState({
        start: new Date(res.start) || new Date(),
        end: new Date(res.end) || new Date(),
        start_time: res.start_time || 9,
        end_time: res.end_time || 18,
        reason: res.reason || 'Congé',
        details: res.details || '',
  ...
  render() {
    return (
      <div>
        ...
        <Typography style={styles.intro}>Modifier les informations concernant une période de congés</Typography>
